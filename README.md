...
| 格式 | JSON + JS |
| 語言 | 繁體中文 |
| 準確度 | 一般 100 筆抽樣驗證通過，另含特殊日期固定 seed 抽樣 |
...
### 特殊日期抽樣驗證（固定 seed：20260610）

本輪額外針對三類特殊日期做可重現抽樣，確認四月十五修正後的資料邏輯沒有誤傷臘月初八，也確認 `衛塞節` 已正確進入前端索引。

| 類型 | 全量筆數 | 抽樣筆數 | 驗證條件 | 結果 |
|------|----------|----------|----------|------|
| 農曆四月十五 | 140 | 5 | `deityBirthday` / `deityInfo` 不含 `釋迦如來成佛日` | ✅ 通過 |
| 農曆臘月初八 | 131 | 5 | `deityBirthday` / `deityInfo` 保留 `釋迦如來成佛日` 與 `臘八節` | ✅ 通過 |
| `衛塞節` | 131 | 5 | `eventId=visakha_bucha`、`nameZh=衛塞節`，且存在前端 `assets/thai-buddhist-days.js` | ✅ 通過 |

抽樣樣本：

- 農曆四月十五：`2013-05-24`、`1981-05-18`、`2019-05-19`、`1999-05-29`、`2034-06-01`
- 農曆臘月初八：`2016-01-17`、`2091-01-27`、`2074-01-05`、`2025-01-07`、`2094-01-24`
- `衛塞節`：`2063-05-12`、`2072-06-01`、`1972-05-27`、`2008-05-19`、`2036-05-10`

補充說明：

- 農曆四月十五抽樣結果皆只剩 `鍾離祖師聖誕`，未再出現 `釋迦如來成佛日`
- 農曆臘月初八抽樣結果皆仍保留 `釋迦如來成佛日` 與 `臘八節`
- `衛塞節` 抽樣結果皆可在 Thai Buddhist 原始資料與前端索引檔中對應到同一日期
